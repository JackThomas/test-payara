...
function ThemeToggleButton(props: ComponentProps<typeof Button>) {
    return (
        <Button
            onClick={(e) => e.preventDefault()}
            variant="secondary"
            size="icon"
            className="hover:shadow-none dark:hover:shadow-none"
            {...props}
        />
    );
}
...
function ThemeToggle() {
    ...
    return (
        <div
            className="fixed top-3 right-3 z-0 flex flex-col gap-1 p-0.75 text-violet-600 dark:text-gray-400 text-center"
            data-testid="theme-toggle"
        >
            <p className="text-xs font-medium">Toggle Theme</p>
            <div className="inline-grid grid-cols-3 gap-1 p-0.75">
                <ThemeToggleButton
                    aria-label="System theme"
                    value="system"
                    onClick={() => onThemeChange("system", setTheme)}
                >
                ...
                <ThemeToggleButton
                    ...
                    value="light"
                    onClick={() => onThemeChange("light", setTheme)}
                >
                    <svg viewBox="0 0 28 28" fill="none">
                ...
                <ThemeToggleButton
                    ...
                    value="dark"
                    onClick={() => onThemeChange("dark", setTheme)}
                >
                    <svg viewBox="0 0 28 28" fill="none">
                        <path
